...
const allowedImageUrl = z.string().url().refine((value) => { const url = new URL(value); return url.protocol === 'https:' && ['dummyjson.com', 'cdn.dummyjson.com', 'cdn.margen.local'].includes(url.hostname); }, 'URL host is not allowed');
